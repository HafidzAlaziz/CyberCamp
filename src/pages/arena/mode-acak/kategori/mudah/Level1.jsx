...
const [realFlag] = useState(() => atob("Q1RGe1NIQURPV19SRUFMTV8yMDIzfQ==")); // CTF{SHADOW_REALM_2023}
const [decoyFlag] = useState(() => atob("Q1RGe0ZBSzVfU0hBRE9XX00zU1M0R0V9")); // CTF{FAKE_SHADOW_MESSAGE}
...
const submitFlag = (e) => {
   ...
   const cleanFlag = inputText.trim();
   if (!cleanFlag) return;
            ...
            <div className="flex gap-4 items-start z-10">
               ...
               <div>
                  <div className="flex items-center gap-2 mb-1">
                     ...
                     <span className="text-[10px] font-black uppercase tracking-[0.4em] text-cyan-400/80">TARGET: SECTOR_OSINT</span>
                  </div>
         ...
         <div className="flex-1 grid grid-cols-12 gap-6 min-h-0">
               ...
               <div className="mt-auto space-y-3">
                  <button
                     onClick={() => {
                        if (!hasUsedHint) {
                           setHasUsedHint(true);
                           localStorage.setItem('ctf_mudah_level1_hint_used', 'true');
                        }
                  ...
                  <AnimatePresence>
                     {showHint && (
                        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="bg-gray-900/50 border border-white/5 rounded-xl p-4 text-[10px] text-gray-500 italic uppercase leading-tight mt-3 overflow-hidden">
                           ...
                           <span className="text-gray-400 block normal-case not-italic">Lihat baik-baik pada pesan terakhir: Nama Project = <span className="text-cyan-300 font-mono">SHADOW_REALM</span>, Tahun = <span className="text-cyan-300 font-mono">2023</span>. Gabungkan dengan format CTF&#123;...&#125;.</span>
                        </motion.div>
                     )}
                  </AnimatePresence>
               </div>
            ...
            <div className="col-span-12 lg:col-span-8 flex flex-col gap-6">
               <div className="flex-1 bg-black border border-cyan-500/20 rounded-2xl flex flex-col overflow-hidden relative group shadow-2xl">
                  ...
                  <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-6">
                     ...
                     <div className="flex items-start gap-4 flex-row-reverse">
                        ...
                        <div className="flex-1 text-right flex flex-col items-end">
                           ...
                           <div className="bg-cyan-950/20 p-3 rounded-2xl rounded-tr-none border border-cyan-500/10 text-xs text-gray-400 leading-relaxed max-w-[85%] relative">
                              Oh, maksud lu project <span className="text-cyan-300 font-bold select-all cursor-crosshair px-1 bg-cyan-500/5 rounded hover:bg-cyan-500/10 transition-colors">"SHADOW_REALM"</span> yang kena takedown tahun <span className="text-cyan-300 font-bold select-all cursor-crosshair px-1 bg-cyan-500/5 rounded hover:bg-cyan-500/10 transition-colors">2023</span> itu?
                              {/* Hidden stub trap */}
                              <div className="absolute top-0 right-0 opacity-0 select-all text-[1px] pointer-events-none">{"CTF{STUB_MESSAGE_DECOY_001}"}</div>
                           </div>
                        </div>
                     </div>
                  ...
                  {/* FLAGS SUBMISSION */}
                  <div className="bg-[#0a0a0a] border-t border-cyan-500/10 p-6 backdrop-blur-xl relative group">
                     <form onSubmit={submitFlag} className="flex flex-col md:flex-row gap-4 relative z-10">
                        <div className="relative flex-1">
                           <input
                              ...
                              className={`w-full bg-black/60 border ${status === 'wrong' || status === 'decoy' ? 'border-red-500 shadow-[0_0_15px_#ef4444]' : 'border-cyan-500/30 focus:border-cyan-500'} text-cyan-100 rounded-xl py-4 px-6 text-xs font-mono focus:outline-none transition-all tracking-widest placeholder:text-gray-700 shadow-[inset_0_2px_10px_rgba(0,0,0,0.5)]`}
                           />
                           <AnimatePresence>
                     ...
                     <div className="mt-4 flex gap-2 overflow-x-auto pb-2 h-8">
                        {attempts.map((att, i) => (
                           <div key={i} className="px-2 py-1 bg-cyan-950/30 border border-cyan-500/20 rounded text-[8px] text-cyan-500/50 italic line-through whitespace-nowrap">{att}</div>
                        ))}
                     </div>
                  </div>
               </div>
         ...
         {showExitModal && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[200] bg-black/80 backdrop-blur-sm flex justify-center items-center p-4">
               <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} className="bg-gray-900 border border-cyan-500/30 rounded-2xl p-8 max-w-md w-full text-center relative overflow-hidden shadow-[0_0_40px_rgba(6,182,212,0.1)]">
                  ...
                  <p className="text-sm text-gray-400 mb-8 font-sans">Anda yakin ingin keluar? Waktu akan terus berjalan dan progress log misi OSINT Anda akan di-reset.</p>
