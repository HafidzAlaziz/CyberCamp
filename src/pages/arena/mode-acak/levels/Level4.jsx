...
const Level4 = () => {
  ...
  const [stars, setStars] = useState(() => {
    ...
    const hasHint = localStorage.getItem('ctf_level4_hint_used') === 'true';
      ...
      <div className="max-w-7xl mx-auto w-full flex-1 flex flex-col relative z-10">
        ...
        <div className="flex justify-between items-start mb-8 relative">
          ...
          <div className="text-right">
            ...
            <div className={`text-4xl font-black italic tracking-tighter transition-colors duration-500 text-emerald-600`}>
              {formatTime(elapsed)}
            </div>
          </div>
        </div>
            ...
            <AnimatePresence>
              {showHint && (
                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="bg-gray-900/50 border border-white/5 rounded-xl p-4 text-[10px] text-gray-500 italic uppercase leading-tight">
                  <span dangerouslySetInnerHTML={{ __html: `<span className="text-purple-300 font-black block mb-2">💡 Apa itu Analisis Paket?</span><span className="text-gray-400 block normal-case not-italic">Data yang dikirim di jaringan dipecah menjadi unit terkecil (paket). Protokol seperti HTTP sering mengirim data dalam bentuk teks polos. Cari paket dengan protokol HTTP dan periksa Payload/Data-nya untuk menemukan informasi kredensial atau Flag.</span>` }} /></motion.div>
              )}
            </AnimatePresence>
          </div>
            ...
            <div className="bg-gray-900/40 border border-emerald-500/30 rounded-2xl p-6 relative overflow-hidden">
               ...
               <form onSubmit={handleSubmit} className="relative z-10 flex flex-col md:flex-row gap-4">
                  <div className="flex-1 relative">
                    <input type="text" value={flag} onChange={(e) => setFlag(e.target.value)} placeholder="Masukkan flag di sini (CTF_{...})" className={`w-full bg-black/60 border-2 rounded-xl py-4 px-6 text-sm tracking-widest text-white placeholder:text-gray-700 focus:outline-none transition-all ${status === 'wrong' || status === 'decoy' ? 'border-red-500 shadow-[0_0_15px_#ef4444]' : 'border-emerald-500/40 focus:border-emerald-500 focus:shadow-[0_0_20px_rgba(16,185,129,0.2)]'}`} />
                    <AnimatePresence>
      ...
      <AnimatePresence>
         {showExitModal && (
            <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
               ...
               <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ opacity: 0 }} className="relative bg-gray-900 border border-emerald-500/30 rounded-3xl p-8 max-w-md w-full text-center shadow-2xl">
