...
const MudahLevel7 = () => {
  /* 
     TRAP_FLAG: <!-- CTF{MEDIA_HTML_TRAP_7070} -->
  */
                    ...
                    {!selectedFile ? (
                      ...
                      <motion.div key="result" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="flex flex-col space-y-1 text-gray-300 w-full pb-4">
                        ...
                        <div className="grid grid-cols-[130px_1fr] md:grid-cols-[180px_1fr] mt-2 border-t border-cyan-500/30 pt-2 pb-2 bg-cyan-950/20 px-2 rounded">
                          ...
                          <span className="text-green-400 font-bold break-words relative">
                             : <span className={`${selectedFile.id === 2 || selectedFile.id === 3 ? "text-red-400" : "text-green-400"} select-all cursor-crosshair px-1 bg-cyan-500/5 rounded hover:bg-cyan-500/10 transition-colors`}>{selectedFile.meta.description}</span>
                             {/* Hidden stub trap */}
                             {selectedFile.id === 1 && <div className="absolute top-0 right-0 opacity-0 select-all text-[1px] pointer-events-none">{"CTF{MEDIA_STUB_DECOY_007}"}</div>}
                          </span>
                        </div>
