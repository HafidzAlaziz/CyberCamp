...
const CryptoLevel8 = () => {
  /* 
     TRAP_FLAG: <!-- CTF{DOUBLE_BASE64_HTML_TRAP_0088} -->
  */
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const correctFlag = atob('Q1RGe0RPVUJMRV9CQVNFNjRfTEFZRVJ9');
    const decoyFlags = [atob('Q1RGe0RPVUJMRV9CQVNFNjRfVFJQNFBfMDA4OH0=')];
  ...
  const saveProgress = (stars) => {
    ...
    const currentStars = stats[8]?.stars || 0;
    const currentBest = stats[8]?.bestTime || 999999;
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-white font-mono p-4 md:p-8 relative overflow-hidden crypto-page-theme">
      {/* BACKGROUND DECORATION - YELLOW THEME FOR LEVEL 8 */}
      ...
      <div className="max-w-7xl mx-auto relative z-10">
          ...
          <div className="flex gap-4 items-start z-10">
            ...
            <div>
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-[10px] font-black uppercase tracking-[0.4em] text-yellow-400/80">TARGET: ENCRYPTED_PAYLOAD</span>
              </div>
              <h1 className="text-3xl font-black text-white italic tracking-tighter uppercase leading-none">
                LEVEL 8: <span className="text-yellow-500 drop-shadow-[0_0_10px_#eab308]">DOUBLE BASE64</span>
              </h1>
            </div>
          </div>
        ...
        <div className="grid grid-cols-12 gap-8">
          ...
          <div className="col-span-12 lg:col-span-4 space-y-6">
            <div className="bg-gray-950/50 border border-yellow-500/30 rounded-2xl p-6 flex-1 flex flex-col relative overflow-hidden group shadow-2xl">
               ...
               <h3 className="text-xl font-black italic tracking-tighter uppercase text-white mb-3">SEKTOR: DOUBLE_ENCODE</h3>
               <p className="text-[11px] text-gray-400 font-bold leading-relaxed mb-6 italic uppercase tracking-wider">
                  Payload yang dicegat ini telah melalui proses encoding dua kali. Lapisan pertama adalah Base64 standar, dan di dalamnya tersembunyi lapisan Base64 kedua yang harus dipecahkan.
               </p>
               ...
               <div className="p-3.5 bg-yellow-500/5 border border-yellow-500/20 rounded-xl mb-6">
                  ...
                  <p className="text-[10px] font-bold text-yellow-100/60 italic tracking-wider leading-relaxed">
                     Decode teks yang ada di panel kanan dua kali menggunakan Base64 untuk menemukan konten aslinya.
                  </p>
               </div>
               ...
               {showHint && (
                 <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} className="mt-3 p-4 bg-black/40 rounded-xl border border-white/5 text-[10px] font-bold text-gray-400 italic tracking-wider leading-relaxed space-y-2">
                    <p className="text-yellow-300 uppercase font-black">💡 Cara Decode Double Base64</p>
                    <p>Buka <span className="text-yellow-400 font-mono">base64decode.org</span> atau tool serupa. Tempel teks yang ada, klik Decode. Kamu akan dapat teks Base64 baru. Lalu decode LAGI untuk mendapatkan isi sesungguhnya.</p>
                    <p className="text-gray-500 mt-2">Lakukan dua kali decode berturut-turut!</p>
                 </motion.div>
               )}
            </div>
            ...
            <div className="bg-black/60 border border-white/5 rounded-3xl p-6 shadow-2xl relative overflow-hidden">
               ...
               <div className="space-y-3 max-h-48 overflow-y-auto custom-scrollbar pr-2">
                  <div className="text-[10px] font-bold tracking-wider italic text-yellow-500/60 uppercase">-- INTEL: Multi-Layer Encoding Detected</div>
                  <div className="text-[10px] font-bold tracking-wider italic text-gray-700 uppercase">-- Encoding Depth: 2x Base64</div>
                  <div className="text-[10px] font-bold tracking-wider italic text-gray-800 uppercase">-- Status: Awaiting Decryption...</div>
               </div>
            </div>
          </div>
            ...
            <div className="flex flex-col flex-1 bg-gray-950 border-2 border-yellow-500/10 rounded-3xl overflow-hidden shadow-2xl relative group">
               ...
               <div className="h-12 bg-yellow-500/5 border-b border-yellow-500/10 flex items-center px-6 gap-3 z-20">
                  ...
                  <div className="flex-1 flex justify-center items-center gap-2">
                     ...
                     <span className="text-[10px] font-black text-yellow-500/50 tracking-widest uppercase">DOUBLE_ENCODED_PAYLOAD</span>
                  </div>
               </div>
               ...
               <div className="flex-1 flex flex-col items-center justify-center p-8 bg-[#05080F] relative">
                  ...
                  <div className="relative z-10 w-full max-w-xl space-y-4">
                    {/* Layer 1 — Double Encoded (what player sees) */}
                    <div className="p-5 bg-yellow-500/5 border border-yellow-500/20 rounded-2xl">
                       <div className="flex items-center gap-2 mb-3">
                          <div className="w-5 h-5 rounded-full bg-yellow-500/20 flex items-center justify-center text-[9px] font-black text-yellow-400">1</div>
                          <span className="text-[9px] font-black text-yellow-500/60 uppercase tracking-widest">Ciphertext — Base64 x2 (Decode dua kali!)</span>
                       </div>
                        <div className="p-4 bg-black/60 rounded-xl border border-white/5 font-mono text-xs text-yellow-400 break-all select-all leading-relaxed hover:bg-yellow-500/5 transition-colors cursor-crosshair relative group">
                           UTFSR2UwUlBWVUpNUlY5Q1FWTkZOalJmVEVGWlJWSjk=
                           {/* Hidden stub trap */}
                           <div className="absolute top-0 right-0 opacity-0 select-all text-[1px] pointer-events-none">{"CTF{DOUBLE_BASE64_STUB_DECOY_808}"}</div>
                        </div>
                    </div>
                    {/* Layer 2 */}
                    <div className="p-5 bg-yellow-500/5 border border-yellow-500/10 rounded-2xl opacity-60">
                       <div className="flex items-center gap-2 mb-3">
                          <div className="w-5 h-5 rounded-full bg-yellow-500/10 flex items-center justify-center text-[9px] font-black text-yellow-500/40">2</div>
                          <span className="text-[9px] font-black text-yellow-500/30 uppercase tracking-widest">Setelah Decode Pertama → Base64 lagi</span>
                       </div>
                       <div className="p-4 bg-black/40 rounded-xl border border-white/5 font-mono text-xs text-gray-700 italic">
                          ??? — Decode Layer 1 dulu!
                       </div>
                    </div>
                    <p className="text-center text-[10px] text-gray-600 font-bold uppercase tracking-widest italic animate-pulse">Double Base64 Encoded Ciphertext — Decode 2x</p>
                  </div>
               </div>
            </div>
                  ...
                  <div className="flex-1 relative">
                    <input 
                       ...
                       className={`w-full bg-black/40 border-2 rounded-xl py-3.5 px-6 text-xs tracking-widest text-white placeholder:text-gray-700 focus:outline-none transition-all ${
                         status === 'wrong' ? 'border-red-500/50 shadow-[0_0_15px_rgba(239,68,68,0.1)]' : status === 'decoy' ? 'border-yellow-500/50 shadow-[0_0_15px_rgba(234,179,8,0.1)]' : 'border-yellow-500/20 focus:border-yellow-500/50'
                       }`} 
                    />
                    <AnimatePresence>
                       {status === 'decoy' && (
        ...
        <div className="mt-8 pt-6 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-4 text-gray-700">
           <div className="flex items-center gap-6 text-[9px] font-black tracking-widest">
              <span className="text-yellow-500/50">{" > "} PROXY: VULNERABLE</span>
              <span className="text-gray-800">//</span>
              <span>NODE: db-fetch-api</span>
              <span className="text-gray-800">//</span>
              <span className="text-white/20 italic text-[8px]">SSRF_PRACTICE</span>
           </div>
           <div className="text-[8px] font-bold uppercase tracking-tighter italic opacity-50">-- NETWORK_BOUNDARIES_COMPROMISED --</div>
        </div>
      </div>
         ...
         {status === 'success' && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 z-[500] bg-[#020617]/90 backdrop-blur-md flex flex-col justify-center items-center p-6 text-center overflow-y-auto crypto-scrollbar-theme">
               ...
               <motion.div initial={{ scale: 0.9, y: 30 }} animate={{ scale: 1, y: 0 }} className="max-w-sm w-full p-8 bg-gray-950 border border-yellow-500/30 rounded-[2.5rem] shadow-[0_0_80px_rgba(234,179,8,0.3)] relative overflow-hidden my-auto">
                  ...
                  <p className="text-yellow-500/60 font-black tracking-[0.4em] uppercase text-[8px] mb-8 italic">Double_Layer_Decrypted</p>
                  ...
                  <button 
                     onClick={() => navigate('/ctf-arena/cryptography/mudah', { state: { lastLevel: 8 } })}
         ...
         {showExitModal && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[600] bg-black/80 backdrop-blur-sm flex justify-center items-center p-4 text-center">
               <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} className="bg-gray-900 border border-yellow-500/30 rounded-3xl p-10 max-w-sm w-full relative overflow-hidden shadow-2xl shadow-yellow-500/10">
                  ...
                  <p className="text-[11px] text-gray-500 mb-10 font-bold leading-relaxed italic uppercase tracking-wider">Anda yakin ingin membatalkan proses dekripsi double layer saat ini?</p>
                  <div className="flex gap-4">
                    ...
                    <button onClick={() => navigate('/ctf-arena/cryptography/mudah', { state: { lastLevel: 8 } })} className="flex-1 py-3.5 bg-red-600 text-white font-black uppercase tracking-widest text-[10px] rounded-xl hover:bg-red-500 transition-colors shadow-[0_0_20px_rgba(239,68,68,0.2)]">KELUAR</button>
                  </div>
               </motion.div>
            </motion.div>
         )}
