...
const CryptoLevel1 = () => {
   ...
   */
   ...
   const handleSubmit = (e) => {
      e.preventDefault();
      const correctFlag = atob('Q1RGe1JPVEFUSU9OX0NBRVNBUn0=');
      const decoyFlags = [atob('Q1RGe1JPVEFUSU9OX1RSUDRQXzAwMTF9')];

      const cleanFlag = flag.trim();
      if (cleanFlag === correctFlag) {
         setStatus('success');
         setIsPaused(true);
         saveProgress(1 + (time <= 120 ? 1 : 0) + (!hintUsed ? 1 : 0));
                     ...
                     <div className="flex-1 flex flex-col items-center justify-center p-8 bg-[#05080F] relative">
                        ...
                        <div className="relative z-10 w-full max-w-lg">
                           <div className="p-6 bg-yellow-500/5 border border-yellow-500/20 rounded-2xl shadow-[0_0_30px_rgba(234,179,8,0.05)] text-center">
                              ...
                              <div className="p-6 bg-black/60 rounded-xl border border-white/5 font-mono text-xl tracking-[0.2em] text-yellow-400 break-all select-all shadow-inner hover:bg-yellow-500/5 transition-colors cursor-crosshair relative group">
                                 FWI&#123;URWDWLRQ_FDHVDU&#125;
                                 {/* Hidden stub trap */}
                                 <div className="absolute top-0 right-0 opacity-0 select-all text-[1px] pointer-events-none">{"CTF{IMPERIAL_STUB_DECOY_101}"}</div>
                              </div>
