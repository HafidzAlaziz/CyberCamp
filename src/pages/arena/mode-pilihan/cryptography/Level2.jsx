...
const CryptoLevel2 = () => {
   /* 
       <!-- CTF{NUMERIC_BASE_HTML_TRAP_0022} -->
   */
   ...
   const projectedStars = 1 + (time <= 180 ? 1 : 0) + (!hintUsed ? 1 : 0);


   const handleSubmit = (e) => {
      e.preventDefault();
      const correctFlag = atob('Q1RGe0JBU0U2NF9JU19BX0NMQVNTSUN9');
      const decoyFlags = [atob('Q1RGe0JBU0U2NF9UUlA0UF8wMDIyfQ==')];
   ...
   const saveProgress = (stars) => {
      ...
      const currentStars = stats[2]?.stars || 0;
      const currentBest = stats[2]?.bestTime || 999999;
   ...
   const cookiesData = [
      { name: 'session_id', value: '7c9e1b2f4a8d3e5c9b0a1r2t3y4u5i6o', secure: true },
      { name: 'user_type', value: 'standard_user', secure: false },
      { name: 'debug_flag', value: 'CTF{C00K1E_M0NST3R_V2}', secure: true },
      { name: 'theme', value: 'dark', secure: false },
      { name: 'language', value: 'id-ID', secure: false }
   ];
   ...
   return (
      <div className="min-h-screen bg-[#020617] text-white font-mono p-4 md:p-8 relative overflow-hidden crypto-page-theme">
         ...
         <div className="absolute inset-0 pointer-events-none">
            ...
            <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-yellow-500/5 blur-[120px] rounded-full animate-pulse" />
         </div>
         ...
         <div className="max-w-7xl mx-auto relative z-10">
               ...
               <div className="flex gap-4 items-start z-10">
                  ...
                  <div>
                     ...
                     <h1 className="text-3xl font-black text-white italic tracking-tighter uppercase leading-none">
                        LEVEL 2: <span className="text-yellow-500 drop-shadow-[0_0_10px_#eab308]">NUMERIC BASE</span>
                     </h1>
                  </div>
               </div>
            ...
            <div className="grid grid-cols-12 gap-8">
               ...
               <div className="col-span-12 lg:col-span-4 space-y-6">
                  <div className="bg-gray-950/50 border border-yellow-500/30 rounded-2xl p-6 flex-1 flex flex-col relative overflow-hidden group shadow-2xl">
                     ...
                     <h3 className="text-xl font-black italic tracking-tighter uppercase text-white mb-3">SEKTOR: NUMERIC_BASE</h3>
                     <p className="text-[11px] text-gray-400 font-bold leading-relaxed mb-6 italic uppercase tracking-wider">
                        Kami menemukan file aneh berisikan susunan huruf dan angka acak. Data ini diyakini menggunakan format encoding "Dasar 64".
                        Dekode susunan karakter tersebut agar terbaca.
                     </p>
                     ...
                     <div className="p-3.5 bg-yellow-500/5 border border-yellow-500/20 rounded-xl mb-6">
                        ...
                        <p className="text-[10px] font-bold text-yellow-100/60 italic tracking-wider leading-relaxed">
                           Lakukan decoding Base64 pada data di panel kanan.
                        </p>
                     </div>
                     ...
                     <button
                        ...
                        className={`w-full py-4 border rounded-xl flex items-center justify-center gap-2.5 transition-all uppercase text-[10px] font-black tracking-widest ${showHint ? 'bg-yellow-500/20 border-yellow-500 text-yellow-400 shadow-[0_0_20px_rgba(234,179,8,0.1)]' : 'bg-white/5 border-white/10 text-gray-500 hover:border-yellow-500/40 hover:text-yellow-400'
                           }`}
                     >
                     ...
                     {showHint && (
                        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} className="mt-3 p-4 bg-black/40 rounded-xl border border-white/5 text-[10px] font-bold text-gray-400 italic tracking-wider leading-relaxed space-y-2">
                           <p className="text-yellow-300 uppercase font-black">💡 Apa itu Base64?</p>
                           <p>Base64 bukanlah sebuah enkripsi, melainkan teknik 'encoding' untuk mengubah data biner atau teks menjadi format standar aman menggunakan (A-Z, a-z, 0-9, +, /). Data Base64 sering berakhiran tanda sama dengan (=).</p>
                           <p className="text-gray-500 mt-2">Anda bisa menggunakan tools online gratis seperti 'Base64 Decode' untuk membalikkan data tersebut ke dalam susunan pesan aslinya.</p>
                        </motion.div>
                     )}
                  </div>
                  ...
                  <div className="bg-black/60 border border-white/5 rounded-3xl p-6 shadow-2xl relative overflow-hidden">
                     ...
                     <div className="space-y-3 max-h-48 overflow-y-auto custom-scrollbar pr-2">
                        <div className="text-[10px] font-bold tracking-wider italic text-yellow-500/60 uppercase">-- INTEL: Cookie storage detected</div>
                        <div className="text-[10px] font-bold tracking-wider italic text-gray-700 uppercase">-- 00:05 -- Scanning local storage...</div>
                        {attempts.map((att, i) => (
                           <div key={i} className="text-[10px] font-bold tracking-wider italic text-red-500/60 uppercase">-- {att} -- INVALID_TOKEN</div>
                        ))}
                     </div>
                  </div>
               </div>
                     ...
                     <div className="flex-1 flex flex-col items-center justify-center p-8 bg-[#05080F] relative">
                        ...
                        <div className="relative z-10 w-full max-w-lg">
                           <div className="p-6 bg-yellow-500/5 border border-yellow-500/20 rounded-2xl shadow-[0_0_30px_rgba(234,179,8,0.05)] text-center">
                              <h4 className="text-[10px] font-black tracking-[0.4em] text-yellow-500/50 uppercase mb-4 italic">Format: 64-bit Radix</h4>
                              <div className="p-6 bg-black/60 rounded-xl border border-white/5 font-mono text-xl tracking-[0.1em] text-yellow-400 break-all select-all shadow-inner hover:bg-yellow-500/5 transition-colors cursor-crosshair relative group">
                                 Q1RGe0JBU0U2NF9JU19BX0NMQVNTSUN9
                                 {/* Hidden stub trap */}
                                 <div className="absolute top-0 right-0 opacity-0 select-all text-[1px] pointer-events-none">{"CTF{BASE64_STUB_DECOY_202}"}</div>
                              </div>

                              <p className="mt-4 text-[10px] text-gray-500 font-bold uppercase tracking-widest italic animate-pulse">Encoding Detected</p>
                           </div>
                        </div>
                     </div>
                  </div>
            ...
            <div className="mt-8 pt-6 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-4 text-gray-700">
               <div className="flex items-center gap-6 text-[9px] font-black tracking-widest">
                  <span className="text-yellow-500/50">{" > "} STORAGE: SCANNING</span>
                  <span className="text-gray-800">//</span>
                  <span>NODE:internal-auth-v2</span>
                  <span className="text-gray-800">//</span>
                  <span className="text-white/20 italic text-[8px]">SESSION_VULN_EXPLOIT</span>
               </div>
               <div className="text-[8px] font-bold uppercase tracking-tighter italic opacity-50">-- OPERATIONAL_OVERLAY_ACTIVE // COOKIE_MONITORING_INIT --</div>
            </div>
         </div>
            ...
            {status === 'success' && (
               <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 z-[500] bg-[#020617]/90 backdrop-blur-md flex flex-col justify-center items-center p-6 text-center overflow-y-auto crypto-scrollbar-theme">
                  ...
                  <motion.div initial={{ scale: 0.9, y: 30 }} animate={{ scale: 1, y: 0 }} className="max-w-sm w-full p-8 bg-gray-950 border border-yellow-500/30 rounded-[2.5rem] shadow-[0_0_80px_rgba(234,179,8,0.3)] relative overflow-hidden my-auto">
                     ...
                     <p className="text-yellow-500/60 font-black tracking-[0.4em] uppercase text-[8px] mb-8 italic">Data_Stream_Interpreted</p>
                     ...
                     <div className="bg-black/40 border border-white/5 rounded-2xl p-5 mb-8 text-left space-y-2">
                        ...
                        <div className="space-y-1.5">
                           ...
                           <div className={`flex items-center justify-between p-3 rounded-xl border transition-all ${time <= 180 ? 'bg-yellow-500/5 border-yellow-500/10 hover:bg-yellow-500/10' : 'bg-red-500/5 border-red-500/10'}`}>
                              <span className={`text-[10px] font-black uppercase italic ${time <= 180 ? 'text-yellow-400' : 'text-gray-600'}`}>
                                 2. Target Waktu (180s)
                              </span>
                              <div className={`w-5 h-5 flex items-center justify-center rounded-lg ${time <= 180 ? 'bg-yellow-500/20' : 'bg-red-500/20'}`}>
                                 {time <= 180 ? (
                                    <CheckCircle2 className="w-3.5 h-3.5 text-yellow-500 shadow-[0_0_10px_#eab308]" />
                                 ) : (
                     ...
                     <button
                        onClick={() => navigate('/ctf-arena/cryptography/mudah', { state: { lastLevel: 2 } })}
         ...
         <AnimatePresence>
            {showExitModal && (
               <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[600] bg-black/80 backdrop-blur-sm flex justify-center items-center p-4 text-center">
                  <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} className="bg-gray-900 border border-yellow-500/30 rounded-3xl p-10 max-w-sm w-full relative overflow-hidden shadow-2xl">
                     <div className="absolute top-0 left-0 w-full h-1 bg-yellow-500" />
                     <ShieldAlert className="w-12 h-12 text-yellow-500 mx-auto mb-6" />
                     <h3 className="text-xl font-black text-white tracking-widest uppercase mb-4 italic">BORT MISSION?</h3>
                     <p className="text-[11px] text-gray-500 mb-10 font-bold leading-relaxed italic uppercase tracking-wider">Anda yakin ingin membatalkan infiltrasi sessi ini?</p>
                     <div className="flex gap-4">
                        ...
                        <button onClick={() => navigate('/ctf-arena/cryptography/mudah', { state: { lastLevel: 2 } })} className="flex-1 py-4 bg-red-600 text-white font-black uppercase tracking-widest text-xs rounded-2xl hover:bg-red-500 transition-colors shadow-[0_0_20px_rgba(239,68,68,0.3)]">Keluar</button>
                     </div>
                  </motion.div>
               </motion.div>
            )}
