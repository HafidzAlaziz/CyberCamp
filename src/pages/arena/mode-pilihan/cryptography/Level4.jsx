...
const CryptoLevel4 = () => {
   /* 
      TRAP_FLAG: <!-- CTF{BINARY_HTML_TRAP_0044} -->
   */
   ...
   // Challenge State
   const [filePath, setFilePath] = useState('document.pdf');
   const [fileContent, setFileContent] = useState(null);

   // Target Time
   const TARGET_TIME = 180; // 3 mins
   ...
   const handleSubmit = (e) => {
      e.preventDefault();
      const correctFlag = atob('Q1RGe0JJTkFSWV9TUEVBS1N9');
      const decoyFlags = [atob('Q1RGe0JJTkFSWV9UUlA0UF8wMDQ0fQ==')];
   ...
   const saveProgress = (stars) => {
      ...
      const currentStars = stats[4]?.stars || 0;
      const currentBest = stats[4]?.bestTime || 999999;
   ...
   return (
      <div className="min-h-screen bg-[#020617] text-white font-mono p-4 md:p-8 relative overflow-hidden crypto-page-theme">
         {/* BACKGROUND DECORATION - CYAN THEME FOR LEVEL 4 */}
         ...
         <div className="max-w-7xl mx-auto relative z-10">
               ...
               <div className="flex gap-4 items-start z-10">
                  ...
                  <div>
                     <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-[0.4em] text-yellow-400/80">TARGET: FILE_SERVER</span>
                     </div>
                     <h1 className="text-3xl font-black text-white italic tracking-tighter uppercase leading-none">
                        LEVEL 4: <span className="text-yellow-500 drop-shadow-[0_0_10px_#eab308]">MACHINE CODE</span>
                     </h1>
                  </div>
               </div>
            ...
            <div className="grid grid-cols-12 gap-8">
               ...
               <div className="col-span-12 lg:col-span-4 space-y-6">
                  <div className="bg-gray-950/50 border border-yellow-500/30 rounded-2xl p-6 flex-1 flex flex-col relative overflow-hidden group shadow-2xl">
                     ...
                     <h3 className="text-xl font-black italic tracking-tighter uppercase text-white mb-3">SEKTOR: MACHINE_CODE</h3>
                     <p className="text-[11px] text-gray-400 font-bold leading-relaxed mb-6 italic uppercase tracking-wider">
                        Kami mengamankan memori rahasia dari sistem utama, namun data ini masih dalam representasi bit fundamental (0 dan 1).
                        Konversikan data ini kembali ke teks yang dapat dibaca. (Spasi memisahkan tiap karakter ASCII).
                     </p>
                     ...
                     <div className="p-3.5 bg-yellow-500/5 border border-yellow-500/20 rounded-xl mb-6">
                        ...
                        <p className="text-[10px] font-bold text-yellow-100/60 italic tracking-wider leading-relaxed">
                           Lakukan konversi Biner (Binary) ke Teks pada data bit yang diberikan.
                        </p>
                     </div>
                     ...
                     {showHint && (
                        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} className="mt-3 p-4 bg-black/40 rounded-xl border border-white/5 text-[10px] font-bold text-gray-400 italic tracking-wider leading-relaxed space-y-2">
                           <p className="text-yellow-300 uppercase font-black">💡 Apa itu Sistem Biner?</p>
                           <p>Sistem bilangan basis dua (Biner) menggunakan kombinasi angka 0 dan 1 untuk merepresentasikan data. Masing-masing blok 8-bit (seperti <span className="text-white font-mono">01000010</span>) mewakili satu huruf dalam standar ASCII.</p>
                           <p className="text-gray-500 mt-2">Gunakan alat online gratis seperti "Binary to Text Converter" untuk mendekode data ini secara otomatis.</p>
                        </motion.div>
                     )}
                  </div>
                  ...
                  <div className="bg-black/60 border border-white/5 rounded-3xl p-6 shadow-2xl relative overflow-hidden">
                     ...
                     <div className="space-y-3 max-h-48 overflow-y-auto custom-scrollbar pr-2">
                        <div className="text-[10px] font-bold tracking-wider italic text-yellow-500/60 uppercase">-- INTEL: File Fetcher Vulnerable detected</div>
                        <div className="text-[10px] font-bold tracking-wider italic text-gray-700 uppercase">-- Base Dir: /var/www/html/public/</div>
                        {attempts.map((att, i) => (
                           <div key={i} className="text-[10px] font-bold tracking-wider italic text-red-500/60 uppercase">-- File Not Found: {att}</div>
                        ))}
                     </div>
                  </div>
               </div>
                     ...
                     <div className="flex-1 flex flex-col items-center justify-center p-8 bg-[#05080F] relative">
                        ...
                        <div className="relative z-10 w-full max-w-lg">
                           <div className="p-6 bg-yellow-500/5 border border-yellow-500/20 rounded-2xl shadow-[0_0_30px_rgba(234,179,8,0.05)] text-center">
                              <h4 className="text-[10px] font-black tracking-[0.4em] text-yellow-500/50 uppercase mb-4 italic">Memory Dump to Binary</h4>
                              <div className="p-6 bg-black/60 rounded-xl border border-white/5 font-mono text-lg font-bold tracking-[0.1em] text-yellow-400 break-all select-all shadow-inner leading-relaxed hover:bg-yellow-500/5 transition-colors cursor-crosshair relative group">
                                 01000010 01001001 01001110 01000001 01010010 01011001 01011111 01010011 01010000 01000101 01000001 01001011 01010011
                                 {/* Hidden stub trap */}
                                 <div className="absolute top-0 right-0 opacity-0 select-all text-[1px] pointer-events-none">{"CTF{BINARY_STUB_DECOY_404}"}</div>
                              </div>

                              <p className="mt-4 text-[10px] text-gray-500 font-bold uppercase tracking-widest italic animate-pulse">Base-2 Encoded</p>
                           </div>
                        </div>
                     </div>
                  </div>
            ...
            <div className="mt-8 pt-6 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-4 text-gray-700">
               <div className="flex items-center gap-6 text-[9px] font-black tracking-widest">
                  <span className="text-yellow-500/50">{" > "} FILE_SYSTEM: INJECT_READY</span>
                  <span className="text-gray-800">//</span>
                  <span>NODE: doc-viewer-v2</span>
                  <span className="text-gray-800">//</span>
                  <span className="text-white/20 italic text-[8px]">DIR_TRAVERSAL_PRACTICE</span>
               </div>
               <div className="text-[8px] font-bold uppercase tracking-tighter italic opacity-50">-- OPERATIONAL_OVERLAY_ACTIVE // PATH_SENTINEL_BYPASS --</div>
            </div>
         </div>
         ...
         {status === 'success' && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 z-[500] bg-[#020617]/90 backdrop-blur-md flex flex-col justify-center items-center p-6 text-center overflow-y-auto crypto-scrollbar-theme">
               ...
               <motion.div initial={{ scale: 0.9, y: 30 }} animate={{ scale: 1, y: 0 }} className="max-w-sm w-full p-8 bg-gray-950 border border-yellow-500/30 rounded-[2.5rem] shadow-[0_0_80px_rgba(234,179,8,0.3)] relative overflow-hidden my-auto">
                  ...
                  <p className="text-yellow-500/60 font-black tracking-[0.4em] uppercase text-[8px] mb-8 italic">Server_Breached</p>
                  ...
                  <button 
                     onClick={() => navigate('/ctf-arena/cryptography/mudah', { state: { lastLevel: 4 } })}
            ...
            {showExitModal && (
               <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[600] bg-black/80 backdrop-blur-sm flex justify-center items-center p-4 text-center">
                  <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} className="bg-gray-900 border border-yellow-500/30 rounded-3xl p-10 max-w-sm w-full relative overflow-hidden shadow-2xl shadow-yellow-500/10">
                     ...
                     <p className="text-[11px] text-gray-500 mb-10 font-bold leading-relaxed italic uppercase tracking-wider">Anda yakin ingin membatalkan pembacaan file saat ini?</p>
                     <div className="flex gap-4">
                        ...
                        <button onClick={() => navigate('/ctf-arena/cryptography/mudah', { state: { lastLevel: 4 } })} className="flex-1 py-3.5 bg-red-600 text-white font-black uppercase tracking-widest text-[10px] rounded-xl hover:bg-red-500 transition-colors shadow-[0_0_20px_rgba(239,68,68,0.2)]">KELUAR</button>
                     </div>
                  </motion.div>
               </motion.div>
            )}
