...
const Level9 = () => {
  ...
  const [stars, setStars] = useState(() => {
    ...
    const currentHint = parseInt(localStorage.getItem('ctf_level9_hint_stage') || '0');
  ...
  if (status === 'complete') {
    return (
      <div className="min-h-screen bg-gray-950 flex flex-col items-center justify-center p-6 font-mono text-white relative overflow-hidden">
        ...
        <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="z-10 bg-gray-900 border-4 border-blue-500 p-12 rounded-3xl text-center max-w-xl shadow-[0_0_50px_rgba(59,130,246,0.3)]">
          <motion.div animate={{ rotate: 360 }} transition={{ duration: 20, repeat: Infinity, ease: "linear" }} className="w-24 h-24 bg-blue-500/10 rounded-full flex items-center justify-center mx-auto mb-8 border border-blue-500/30">
            <Cpu className="w-12 h-12 text-blue-400 drop-shadow-[0_0_10px_#3b82f6]" />
          </motion.div>
          <h1 className="text-5xl font-black italic tracking-tighter mb-2 text-white uppercase leading-none">MISSION COMPLETE</h1>
          <p className="text-blue-500 font-bold tracking-[0.3em] mb-4 text-[10px]">SECTOR: LOGIC_GATE // ACCESS_GRANTED</p>
          <div className="bg-blue-500/5 border border-blue-500/10 rounded-2xl py-6 px-10 mb-8 inline-block">
             ...
             <div className="flex justify-center gap-4">
               {[1, 2, 3, 4].map(s => (
                 <Zap key={s} className={`w-10 h-10 transition-all duration-500 ${s <= stars ? 'text-blue-400 fill-blue-400 drop-shadow-[0_0_15px_#3b82f6]' : 'text-gray-800 fill-transparent'}`} />
               ))}
             </div>
          </div>
          <button onClick={() => navigate('/ctf-arena/mode-acak', { state: { returnToLevel: 9 } })} className="w-full bg-blue-500 text-black font-black py-4 rounded-xl hover:bg-blue-400 transition-all text-sm tracking-widest uppercase shadow-[0_10px_30px_rgba(59,130,246,0.2)]">KEMBALI KE MAP TAKTIS</button>
        </motion.div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-gray-300 font-mono p-4 md:p-8 flex flex-col overflow-hidden relative">
      <div className="absolute inset-0 opacity-20 pointer-events-none" style={{ backgroundImage: `radial-gradient(circle at 2px 2px, #172554 1px, transparent 0)`, backgroundSize: '40px 40px' }} />
      <div className="max-w-7xl mx-auto w-full flex-1 flex flex-col relative z-10">
        ...
        <div className="flex justify-between items-start mb-8 relative">
          <div className="flex gap-4 items-start z-10">
             <button 
                ...
                className="mt-1 p-2 bg-gray-900 border border-blue-500/30 rounded-xl hover:bg-blue-500/20 text-gray-400 hover:text-blue-400 transition-all group"
                title="Abort Mission"
             >
             ...
             <div>
                <div className="flex items-center gap-2 mb-1">
                   ...
                   <span className="text-[10px] font-black uppercase tracking-[0.4em] text-blue-400/80">TARGET: SECTOR_HARDWARE</span>
                </div>
                <h1 className="text-3xl font-black text-white italic tracking-tighter uppercase leading-none">
                   LEVEL 9: <span className="text-blue-600 drop-shadow-[0_0_10px_#3b82f6]">REVERSE ENGINEERING</span>
                </h1>
             </div>
          </div>
          ...
          <div className="absolute left-1/2 -translate-x-1/2 top-0 flex flex-col items-center">
             ...
             <div className="flex gap-3">
                {[1, 2, 3, 4].map(s => (
                  <Zap key={s} className={`w-6 h-6 transition-all duration-700 ${s <= stars ? 'text-blue-400 fill-blue-400 drop-shadow-[0_0_10px_#3b82f6]' : 'text-white/10 fill-transparent opacity-20'}`} />
                ))}
             </div>
          </div>
          ...
          <div className="text-right">
             ...
             <div className="text-4xl font-black italic tracking-tighter transition-colors duration-500 text-blue-600">
                {formatTime(elapsed)}
             </div>
          </div>
        </div>
        ...
        <div className="flex-1 grid grid-cols-12 gap-6 min-h-0">
          ...
          <div className="col-span-12 lg:col-span-4 flex flex-col gap-6">
             <div className="bg-gray-950/50 border border-blue-500/30 rounded-2xl p-6 flex-1 flex flex-col relative overflow-hidden group shadow-[inset_0_0_20px_rgba(59,130,246,0.05)]">
                <div className="absolute top-0 left-0 w-1 h-full bg-blue-500 shadow-[0_0_15px_#3b82f6]" />
                <div className="flex items-center gap-3 mb-6">
                   <Terminal className="w-5 h-5 text-blue-400" />
                   <h2 className="text-sm font-black text-blue-200 uppercase tracking-widest">BRIEFING MISI</h2>
                </div>
                <div className="text-xs leading-relaxed text-gray-400 overflow-y-auto pr-2 custom-scrollbar space-y-4 italic mb-8">
                   ...
                   <div className="p-3 bg-blue-950/20 border border-blue-500/20 rounded-lg">
                      <p className="text-blue-400 font-black tracking-tight uppercase mb-1 flex items-center gap-2">
                ...
                <div className="mt-auto space-y-3">
                   <div className="p-3 bg-white/5 rounded-xl border border-white/5 flex items-center justify-between">
                      ...
                      <span className="text-[9px] font-bold text-blue-500">ENCRYPTED</span>
                   </div>
                   <button onClick={() => setShowHintModal(true)} className={`bg-emerald-900/20 border rounded-xl p-4 flex items-center justify-center gap-3 group transition-all shadow-[0_0_20px_rgba(16,185,129,0.1)] ${hintStage > 0 ? 'border-emerald-400 bg-emerald-900/40' : 'border-emerald-500/40 hover:bg-emerald-900/30'}`}>
              <Zap className={`w-4 h-4 transition-colors ${hintStage > 0 ? 'text-emerald-400 fill-emerald-400' : 'text-emerald-400'}`} />
              <span className="text-xs font-black text-emerald-200 uppercase tracking-widest uppercase">💡 MINTA HINT BOS ({hintStage}/3)</span>
            </button>
                </div>
             </div>
          </div>
          ...
          <div className="col-span-12 lg:col-span-8 flex flex-col gap-6">
             <div className="flex-1 bg-black border border-blue-500/20 rounded-2xl flex flex-col overflow-hidden relative shadow-2xl">
                ...
                <div className="flex-1 p-8 flex flex-col items-center justify-center space-y-12 overflow-y-auto custom-scrollbar">
                   ...
                   <div className="bg-[#050505] border border-white/5 rounded-lg p-6 max-w-2xl w-full font-mono relative shadow-2xl">
                      ...
                      <div className="space-y-1 text-[13px] leading-relaxed">
                         ...
                         <div className="flex gap-8">
                            ...
                            <div>
                               <span className="text-[#a855f7] font-bold">MOV</span>
                               <span className="text-[#22d3ee] ml-4">EAX</span>, 
                         ...
                         <div className="flex gap-8">
                            ...
                            <div>
                               <span className="text-[#a855f7] font-bold">MOV</span>
                               <span className="text-[#22d3ee] ml-4">EBX</span>, 
                         ...
                         <div className="flex gap-8">
                            ...
                            <div>
                               <span className="text-[#a855f7] font-bold">XOR</span>
                               <span className="text-[#22d3ee] ml-4">EAX</span>, 
                               <span className="text-[#22d3ee] ml-2">EBX</span>
                         ...
                         <div className="flex gap-8">
                            ...
                            <div>
                               <span className="text-[#a855f7] font-bold">ADD</span>
                               <span className="text-[#22d3ee] ml-4">EAX</span>, 
                         ...
                         <div className="flex gap-8">
                            ...
                            <div>
                               <span className="text-[#a855f7] font-bold">RET</span>
                      ...
                      <div className="mt-12 flex flex-col gap-4 max-w-xs mx-auto">
                         <div className="relative">
                            <input
                               ...
                               className="w-full bg-black/40 border border-white/5 rounded-lg py-3 px-4 text-center text-lg font-mono text-blue-100 focus:outline-none focus:border-blue-500/50 transition-all uppercase placeholder:text-gray-800"
                            />
                         </div>
                         <button onClick={checkCalc} className="bg-blue-600/10 hover:bg-blue-600/20 border border-blue-500/30 text-blue-400 font-bold py-3 rounded-lg text-[10px] tracking-widest uppercase transition-all active:scale-95">
                   ...
                   <AnimatePresence>
                      {unlockStatus === 'unlocked' && (
                         <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full bg-blue-950/20 border-2 border-blue-500/40 rounded-2xl p-6 text-center relative">
                            <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-4 bg-blue-500 text-black text-[9px] font-black italic rounded-full uppercase py-0.5 shadow-[0_0_15px_#3b82f6]">ACCESS_UNLOCKED</div>
                            <div className="text-gray-400 font-mono text-xs mb-4">Decrypted flag found in hardware memory:</div>
                            <div className="bg-black/40 p-4 rounded-xl border border-white/5 font-mono text-blue-400 font-bold tracking-widest break-all select-all">
                               {realFlag}
                            </div>
                         </motion.div>
                      )}
                ...
                <div className="bg-gray-900 border-t border-blue-500/20 p-6 backdrop-blur-md">
                   <div className="text-[9px] font-black text-blue-500/50 uppercase tracking-[0.3em] mb-4">Enter decrypted credentials:</div>
                   <form onSubmit={submitFlag} className="flex flex-col md:flex-row gap-4">
                      <div className="relative flex-1 group">
                         <Lock className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-blue-500/30 group-focus-within:text-blue-500 transition-colors" />
                         <input
                            type="text"
                            value={inputText}
                            onChange={(e) => setInputText(e.target.value)}
                            placeholder="CTF{...}"
                            className={`w-full bg-black/60 border ${status === 'wrong' ? 'border-red-500 shadow-[0_0_15px_#ef4444]' : 'border-blue-500/30 focus:border-blue-500'} rounded-xl py-4 pl-12 pr-4 text-xs font-mono text-white focus:outline-none transition-all tracking-widest`}
                         />
                      </div>
                      <button type="submit" className="bg-blue-600 hover:bg-blue-500 text-white font-black px-12 py-4 rounded-xl text-xs tracking-widest uppercase transition-all shadow-lg active:scale-95 flex items-center justify-center gap-2">
        ...
        <div className="mt-8 pt-6 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-4 text-gray-700">
           <div className="flex items-center gap-4 text-[9px] font-black tracking-widest">
              <span className="text-blue-500/50">{" > "} HW_LINK: STABLE</span>
      ...
      <AnimatePresence>
        {showHintModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="relative bg-gray-900 border border-blue-500/30 rounded-3xl p-8 max-w-lg w-full shadow-[0_0_50px_rgba(59,130,246,0.2)] overflow-hidden">
               <div className="absolute top-0 left-0 w-full h-1 bg-blue-500" />
               <div className="flex justify-between items-start mb-8">
                  <div>
                    <h2 className="text-2xl font-black text-white italic tracking-tighter uppercase mb-1">LOGIC_RECOVERY_HUB</h2>
                    <p className="text-[10px] text-gray-500 font-bold tracking-widest uppercase">Sector 9: Hardware Logic Analysis</p>
                  </div>
               ...
               <div className="space-y-4 mb-8">
                  {[
                    { depth: 1, label: "ANALYZING_BITS", content: `<span class="text-purple-300 font-black block mb-2">💡 Apa itu Reverse Engineering?</span><span class="text-gray-400 block normal-case not-italic">Proses membongkar sistem untuk memahami cara kerjanya. Dalam tantangan ini, kamu harus memecahkan logika gerbang biner dan hexadecimal. Fokus pada operasi XOR: <span class="text-purple-300 font-mono">15 ^ 5 = 10</span> (Hasilnya 10 dalam desimal).</span>` },
                    { depth: 2, label: "CONVERTING_DATA", content: "Operasi logika tersebut menghasilkan angka 30 dalam desimal. Carilah padanan hexadecimal-nya (0xXX)." },
                    { depth: 3, label: "MASTER_LOGIC", content: "Hasil konversi desimal 30 ke Hex adalah <span class='text-blue-500 font-mono font-black'>0x1E</span>. Gunakan itu untuk membuka admin access." }
                  ].map((h, i) => (
                    <div key={i} className={`p-4 rounded-2xl border transition-all ${hintStage >= h.depth ? 'bg-blue-500/10 border-blue-500/40 text-gray-200' : 'bg-black/40 border-white/5 text-gray-600'}`}>
                       <div className="flex justify-between items-center mb-2">
                          <span className="text-[10px] font-black tracking-widest uppercase">{h.label}</span>
                          {hintStage >= h.depth ? <Check className="w-3 h-3 text-blue-500" /> : <Lock className="w-3 h-3" />}
                       </div>
                       {hintStage >= h.depth ? (
                         <p className="text-xs leading-relaxed italic" dangerouslySetInnerHTML={{ __html: h.content }} />
                       ) : (
                         <button onClick={() => unlockHintStage(h.depth)} className="w-full py-2 bg-blue-600 hover:bg-blue-500 text-white text-[10px] font-black uppercase tracking-widest rounded-lg transition-all">UNLOCK LOGIC DEPTH {h.depth}</button>
                       )}
                    </div>
                  ))}
               </div>
        ...
        {showExitModal && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
             ...
             <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ opacity: 0 }} className="relative bg-gray-900 border border-blue-500/30 rounded-3xl p-8 max-w-md w-full text-center shadow-2xl">
