...
const CryptoLevel9 = () => {
  /* 
     TRAP_FLAG: <!-- CTF{VIGENERE_HTML_TRAP_0099} -->
  */
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const correctFlag = atob('Q1RGe1ZJR0VORVJFX0tFWV9DUkFDS0VEfQ==');
    const decoyFlags = [atob('Q1RGe1ZJR0VORVJFX1RSUDRQXzAwOTl9')];
  ...
  const saveProgress = (stars) => {
    ...
    const currentStars = stats[9]?.stars || 0;
    const currentBest = stats[9]?.bestTime || 999999;
    stats[9] = { 
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-white font-mono p-4 md:p-8 relative overflow-hidden crypto-page-theme">
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-0 left-0 w-full h-full bg-[linear-gradient(rgba(234,179,8,0.03)_1px,transparent_1px),linear-gradient(90deg,rgba(234,179,8,0.03)_1px,transparent_1px)] bg-[size:40px_40px]" />
        <div className="absolute top-1/4 right-1/4 w-96 h-96 bg-yellow-500/5 blur-[120px] rounded-full animate-pulse" />
      </div>
      ...
      <div className="max-w-7xl mx-auto relative z-10">
          ...
          <div className="flex gap-4 items-start z-10">
            ...
            <div>
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-[10px] font-black uppercase tracking-[0.4em] text-yellow-400/80">TARGET: ENCRYPTED_TRANSMISSION</span>
              </div>
              <h1 className="text-3xl font-black text-white italic tracking-tighter uppercase leading-none">
                LEVEL 9: <span className="text-yellow-500 drop-shadow-[0_0_10px_#eab308]">VIGENERE CIPHER</span>
              </h1>
            </div>
          </div>
             ...
             <div className="flex gap-2">
                {[1, 2, 3].map(s => (
                   <div key={s} className={`w-2 h-6 rounded-sm border transition-all duration-500 ${
                     s <= projectedStars
                       ? 'bg-yellow-400 border-yellow-300 shadow-[0_0_8px_rgba(234,179,8,0.8)]'
        ...
        <div className="grid grid-cols-12 gap-8">
          ...
          <div className="col-span-12 lg:col-span-4 space-y-6">
            <div className="bg-gray-950/50 border border-yellow-500/30 rounded-2xl p-6 flex-1 flex flex-col relative overflow-hidden group shadow-2xl">
               ...
               <h3 className="text-xl font-black italic tracking-tighter uppercase text-white mb-3">SEKTOR: KEYWORD_CIPHER</h3>
               <p className="text-[11px] text-gray-400 font-bold leading-relaxed mb-6 italic uppercase tracking-wider">
                  Transmisi yang dicegat ini dienkripsi menggunakan metode sandi Vigenere — sandi polialfabetik yang menggunakan sebuah kata kunci (key) untuk menggeser setiap huruf secara berbeda-beda.
               </p>
               ...
               <div className="p-3.5 bg-yellow-500/5 border border-yellow-500/20 rounded-xl mb-6">
                  ...
                  <p className="text-[10px] font-bold text-yellow-100/60 italic tracking-wider leading-relaxed">
                     Ciphertext: <span className="text-yellow-400 font-mono">XGHIEGPF_OVA_ASETMCE</span><br/>
                     Key (tersedia): <span className="text-yellow-300 font-mono">CYBER</span><br/>
                     Pecahkan cipher menggunakan key yang tersedia untuk mengungkap flag.
                  </p>
               </div>
               ...
               {showHint && (
                 <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} className="mt-3 p-4 bg-black/40 rounded-xl border border-white/5 text-[10px] font-bold text-gray-400 italic tracking-wider leading-relaxed space-y-2">
                    <p className="text-yellow-300 uppercase font-black">💡 Cara Decode Vigenere</p>
                    <p>Vigenere menggunakan key untuk menggeser karakter. Setiap huruf di ciphertext digeser mundur sesuai posisi huruf key yang berulang.</p>
                    <p className="text-gray-500 mt-2">Gunakan tool online seperti <span className="text-yellow-400 font-mono">cryptii.com</span> atau <span className="text-yellow-400 font-mono">dcode.fr/vigenere-cipher</span> dengan key <span className="text-white">CYBER</span>.</p>
                 </motion.div>
               )}
            </div>
            ...
            <div className="bg-black/60 border border-white/5 rounded-3xl p-6 shadow-2xl relative overflow-hidden">
               ...
               <div className="space-y-3 max-h-48 overflow-y-auto custom-scrollbar pr-2">
                  <div className="text-[10px] font-bold tracking-wider italic text-yellow-500/60 uppercase">-- INTEL: Polyalphabetic Cipher Detected</div>
                  <div className="text-[10px] font-bold tracking-wider italic text-gray-700 uppercase">-- Key Length Estimate: 5 chars</div>
                  <div className="text-[10px] font-bold tracking-wider italic text-gray-800 uppercase">-- Awaiting Decryption...</div>
               </div>
            </div>
          </div>
            ...
            <div className="flex flex-col flex-1 bg-gray-950 border-2 border-yellow-500/10 rounded-3xl overflow-hidden shadow-2xl relative group">
               ...
               <div className="h-12 bg-yellow-500/5 border-b border-yellow-500/10 flex items-center px-6 gap-3 z-20">
                  ...
                  <div className="flex-1 flex justify-center items-center gap-2">
                     <KeyRound className="w-3.5 h-3.5 text-yellow-500/50" />
                     <span className="text-[10px] font-black text-yellow-500/50 tracking-widest uppercase">VIGENERE_ENCRYPTED_TRANSMISSION</span>
                  </div>
               </div>
               ...
               <div className="flex-1 flex flex-col items-center justify-center p-8 bg-[#05080F] relative">
                  ...
                  <div className="relative z-10 w-full max-w-xl space-y-5">
                    {/* Ciphertext Block */}
                    <div className="p-6 bg-yellow-500/5 border border-yellow-500/20 rounded-2xl shadow-[0_0_30px_rgba(234,179,8,0.05)]">
                       <div className="flex items-center gap-2 mb-4">
                          <div className="w-5 h-5 rounded-full bg-yellow-500/20 flex items-center justify-center">
                             <KeyRound className="w-3 h-3 text-yellow-400" />
                          </div>
                          <span className="text-[9px] font-black text-yellow-500/60 uppercase tracking-widest">Intercepted Ciphertext</span>
                       </div>
                        <div className="p-5 bg-black/60 rounded-xl border border-white/5 font-mono text-2xl md:text-3xl font-bold tracking-[0.15em] text-yellow-400 text-center select-all shadow-inner hover:bg-yellow-500/5 transition-colors cursor-crosshair relative group">
                           XGHIEGPF_OVA_ASETMCE
                           {/* Hidden stub trap */}
                           <div className="absolute top-0 right-0 opacity-0 select-all text-[1px] pointer-events-none">{"CTF{VIGENERE_STUB_DECOY_909}"}</div>
                        </div>
                    </div>
                    {/* Key Block */}
                    <div className="p-5 bg-yellow-500/5 border border-yellow-500/10 rounded-2xl">
                       <div className="flex items-center gap-2 mb-3">
                          <span className="text-[9px] font-black text-yellow-500/40 uppercase tracking-widest">Key (Recovered from Metadata)</span>
                       </div>
                       <div className="p-4 bg-black/40 rounded-xl border border-yellow-500/10 font-mono text-xl font-bold tracking-[0.3em] text-yellow-500/80 text-center select-all">
                          CYBER
                       </div>
                    </div>
                    <p className="text-center text-[10px] text-gray-600 font-bold uppercase tracking-widest italic animate-pulse">
                       Vigenere Ciphertext — Key: CYBER — Decode untuk temukan flag
                    </p>
                  </div>
               </div>
            </div>
                  ...
                  <div className="flex-1 relative">
                    <input 
                       ...
                       className={`w-full bg-black/40 border-2 rounded-xl py-3.5 px-6 text-xs tracking-widest text-white placeholder:text-gray-700 focus:outline-none transition-all ${
                         status === 'wrong' ? 'border-red-500/50 shadow-[0_0_15px_rgba(239,68,68,0.1)]' : status === 'decoy' ? 'border-yellow-500/50 shadow-[0_0_15px_rgba(234,179,8,0.1)]' : 'border-yellow-500/25 focus:border-yellow-500/60 focus:shadow-[0_0_20px_rgba(234,179,8,0.1)]'
                       }`} 
                    />
                    <AnimatePresence>
                       {status === 'decoy' && (
        ...
        <div className="mt-8 pt-6 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-4 text-gray-700">
           <div className="flex items-center gap-6 text-[9px] font-black tracking-widest">
              <span className="text-yellow-500/50">{" > "} CIPHER: VIGENERE_POLY</span>
              <span className="text-gray-800">//</span>
              <span>NODE: crypto-layer-9</span>
              <span className="text-gray-800">//</span>
              <span className="text-white/20 italic text-[8px]">VIGENERE_PRACTICE</span>
           </div>
           <div className="text-[8px] font-bold uppercase tracking-tighter italic opacity-50">-- KEY_ASSISTED_DECRYPTION_REQUIRED --</div>
        </div>
      </div>
         ...
         {status === 'success' && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 z-[500] bg-[#020617]/90 backdrop-blur-md flex flex-col justify-center items-center p-6 text-center overflow-y-auto crypto-scrollbar-theme">
               ...
               <motion.div initial={{ scale: 0.9, y: 30 }} animate={{ scale: 1, y: 0 }} className="max-w-sm w-full p-8 bg-gray-950 border border-yellow-500/30 rounded-[2.5rem] shadow-[0_0_80px_rgba(234,179,8,0.3)] relative overflow-hidden my-auto">
                  ...
                  <p className="text-yellow-500/60 font-black tracking-[0.4em] uppercase text-[8px] mb-8 italic">Vigenere_Cracked</p>
                  ...
                  <button 
                     onClick={() => navigate('/ctf-arena/cryptography/mudah', { state: { lastLevel: 9 } })}
      ...
      <AnimatePresence>
         {showExitModal && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[600] bg-black/80 backdrop-blur-sm flex justify-center items-center p-4 text-center">
               <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} className="bg-gray-900 border border-yellow-500/30 rounded-3xl p-10 max-w-sm w-full relative overflow-hidden shadow-2xl shadow-yellow-500/10">
                  <div className="absolute top-0 left-0 w-full h-1 bg-yellow-500" />
                  <ShieldAlert className="w-12 h-12 text-yellow-500 mx-auto mb-6" />
                  <h3 className="text-xl font-black text-white tracking-widest uppercase mb-4 italic">ABORT MISSION?</h3>
                  <p className="text-[11px] text-gray-500 mb-10 font-bold leading-relaxed italic uppercase tracking-wider">Anda yakin ingin membatalkan dekripsi Vigenere saat ini?</p>
                  <div className="flex gap-4">
                    ...
                    <button onClick={() => navigate('/ctf-arena/cryptography/mudah', { state: { lastLevel: 9 } })} className="flex-1 py-3.5 bg-red-600 text-white font-black uppercase tracking-widest text-[10px] rounded-xl hover:bg-red-500 transition-colors shadow-[0_0_20px_rgba(239,68,68,0.2)]">KELUAR</button>
                  </div>
               </motion.div>
            </motion.div>
         )}
