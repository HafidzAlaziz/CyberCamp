...
const MenengahLevel3 = () => {
  ...
  const hintPenalty = Math.min(2, hintStage);
  ...
  const TOLERANCE = 5; // Â±5 units wiggle room
  ...
  const verifyFlag = (e) => {
    ...
    if (flagInput.trim() === TRUE_FLAG) {
      ...
      const currentBestStars = stats['menengah-3']?.stars || 0;
      const currentBestTimeSec = (() => { 
        const t = stats['menengah-3']?.bestTime; 
        if (!t) return 999999;
      ...
      if (isBetter) {
         stats['menengah-3'] = { stars: stars, bestTime: timeTakenStr, conditions: [true, hintStage < 1, hintStage < 2, !isTimeFailed] };
         localStorage.setItem('ctf_mode_acak_stats', JSON.stringify(stats));
      }
      localStorage.removeItem('ctf_menengah_level3_time');
      localStorage.removeItem('ctf_menengah_level3_stars');
      localStorage.removeItem('ctf_menengah_level3_hint_stage');
      localStorage.removeItem('ctf_menengah_level3_overtime');
    } else if (flagInput !== "") {
      setStatus('wrong');
  ...
  const unlockHintStage = (stage) => {
    ...
    setHintStage(stage);
    localStorage.setItem('ctf_menengah_level3_hint_stage', stage.toString());
  };
  ...
  const handleExit = () => {
    ...
    localStorage.removeItem('ctf_menengah_level3_hint_stage');
    localStorage.removeItem('ctf_menengah_level3_overtime');
    navigate('/ctf-arena/mode-acak', { state: { returnToLevel: 'menengah-3' } });
  };
      ...
      <div className="min-h-screen bg-gray-950 flex flex-col items-center justify-center p-6 font-mono text-white relative overflow-hidden">
        ...
        <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="z-10 bg-gray-900 border-4 border-yellow-500 p-12 rounded-3xl text-center max-w-xl shadow-[0_0_50px_rgba(234,179,8,0.3)]">
          <motion.div animate={{ rotate: 360 }} transition={{ duration: 20, repeat: Infinity, ease: "linear" }} className="w-24 h-24 bg-yellow-500/10 rounded-full flex items-center justify-center mx-auto mb-8 border border-yellow-500/30">
            <ScanSearch className="w-12 h-12 text-yellow-400 drop-shadow-[0_0_10px_#eab308]" />
          </motion.div>
          <h1 className="text-5xl font-black italic tracking-tighter mb-2 text-white uppercase leading-none">MISSION COMPLETE</h1>
          <p className="text-yellow-400 font-bold tracking-[0.3em] mb-4 text-[10px]">SECTOR: MEDIA_CORE // DATA_UNCOVERED</p>
          <div className="bg-yellow-400/5 border border-yellow-400/10 rounded-2xl py-6 px-10 mb-8 inline-block">
          ...
          <button onClick={() => navigate('/ctf-arena/mode-acak', { state: { returnToLevel: 'menengah-3' } })} className="w-full bg-yellow-500 text-black font-black py-4 rounded-xl hover:bg-yellow-400 transition-all text-sm tracking-widest uppercase shadow-[0_10px_30px_rgba(234,179,8,0.2)]">KEMBALI KE MAP TAKTIS</button>
        </motion.div>
      </div>
    );
  }
      ...
      <div className="max-w-7xl mx-auto w-full flex-1 flex flex-col relative z-10">
        ...
        <div className="flex justify-between items-start mb-8 relative">
          <div className="flex gap-4 items-start z-10">
            ...
            <div>
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-[10px] font-black uppercase tracking-[0.4em] text-yellow-400/80">TARGET: SECTOR_MEDIA</span>
              </div>
              <h1 className="text-3xl font-black text-white italic tracking-tighter uppercase leading-none">
                LEVEL 3: <span className="text-yellow-400 drop-shadow-[0_0_10px_#eab308]">STEGANOGRAFI MEDIA</span>
              </h1>
            </div>
          </div>
          ...
          <div className="text-right">
            ...
            <div className={`text-4xl font-black italic tracking-tighter transition-colors duration-500 ${isTimeFailed ? 'text-red-500 animate-pulse' : 'text-yellow-400'}`}>
              {formatTime(elapsed)}
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 grid grid-cols-12 gap-6 min-h-0">
          ...
          <div className="col-span-12 lg:col-span-4 flex flex-col gap-6">
            <div className="bg-gray-950/50 border border-yellow-500/30 rounded-2xl p-6 flex-1 flex flex-col relative overflow-hidden group shadow-[inset_0_0_20px_rgba(234,179,8,0.05)]">
              ...
              {/* SLIDERS CONTROLS */}
              <div className="mt-auto flex flex-col gap-5 bg-black/40 p-4 rounded-xl border border-yellow-900/40">
                <div className="text-[10px] font-black text-yellow-500 uppercase tracking-widest border-b border-yellow-500/20 pb-2 mb-2">SPECTRUM TUNER V.2</div>
                ...
                {[
                   ...
                   <div key={slider.stateKey}>
                      <div className="flex justify-between text-[10px] text-gray-500 mb-1">
                         ...
                         <span className={Math.abs(filters[slider.stateKey] - slider.target) <= TOLERANCE ? 'text-yellow-400 font-bold drop-shadow-[0_0_5px_rgba(234,179,8,0.5)]' : ''}>
                      ...
                      <input 
                         ...
                         className="w-full h-1 bg-gray-800 rounded-lg appearance-none cursor-pointer accent-yellow-500 shadow-[0_0_10px_#eab308]"
                      />
                   </div>
                ))}
              </div>
            ...
            <div className="mt-auto">
               <button 
                  ...
                  className={`w-full border rounded-xl p-4 flex items-center justify-center gap-3 group transition-all shadow-[0_0_20px_rgba(234,179,8,0.1)] ${hintStage > 0 ? 'border-yellow-400 bg-yellow-950/40 text-yellow-400' : 'border-yellow-500/40 bg-yellow-950/20 hover:bg-yellow-950/30 text-yellow-200'}`}
               >
                  <Zap className={`w-4 h-4 transition-colors ${hintStage > 0 ? 'fill-yellow-400' : ''}`} />
                  <span className="text-xs font-black uppercase tracking-widest">
                     {hintStage > 0 ? `HINT ACTIVE (${hintStage}/2)` : '💡 MINTA PETUNJUK (Hingga -2 ⚡)'}
                  </span>
               </button>
            </div>
          </div>
          ...
          <div className="col-span-12 lg:col-span-8 flex flex-col gap-6">
            ...
            <div className="relative flex-1 bg-black border border-yellow-500/20 rounded-2xl overflow-hidden shadow-[0_0_30px_rgba(0,0,0,0.5)] flex items-center justify-center min-h-[400px]">
               ...
               <div className="relative z-10 w-full h-full flex flex-col items-center justify-center p-8 text-center font-bold tracking-[0.8em] md:text-2xl text-xl break-all">
                  ...
                  {isCompleteMatch && (
                     <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="absolute -top-10 bg-yellow-500 text-black text-[10px] tracking-widest px-4 py-1 rounded-full uppercase font-black mb-10 shadow-[0_0_20px_#eab308]">
                       DECRYPTION 100% MATCH
                     </motion.div>
                  )}

                  <span className={`transition-all duration-75 text-center block ${isCompleteMatch ? 'text-yellow-400 drop-shadow-[0_0_15px_#eab308]' : 'text-yellow-900/30 blur-[2px]'}`}>
                     {renderedText}
                  </span>
               </div>
            ...
            {/* FLAG INPUT BOX */}
            <div className={`bg-gray-950/80 border rounded-2xl p-6 relative overflow-hidden transition-all duration-500 ${status === 'wrong' ? 'border-red-500 shadow-[0_0_30px_rgba(239,68,68,0.2)]' : 'border-yellow-500/30'}`}>
              ...
              <form onSubmit={verifyFlag} className="flex gap-4">
                <div className="flex-1 relative">
                  ...
                  <input
                    ...
                    className={`w-full bg-gray-900 border ${status === 'wrong' ? 'border-red-500/50 text-red-100' : 'border-yellow-500/20 text-yellow-100 focus:border-yellow-400'} rounded-xl py-4 pl-12 pr-4 outline-none font-mono text-sm tracking-wider transition-all shadow-[inset_0_2px_10px_rgba(0,0,0,0.3)]`}
                  />
                </div>
                <button type="submit" className="bg-yellow-600 hover:bg-yellow-500 text-white px-8 rounded-xl font-black tracking-widest uppercase transition-all flex items-center gap-2 hover:shadow-[0_0_20px_rgba(234,179,8,0.4)]">
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="relative bg-gray-900 border border-yellow-500/30 rounded-3xl p-8 max-w-lg w-full shadow-[0_0_50px_rgba(234,179,8,0.2)] overflow-hidden">
               ...
               <div className="space-y-4 mb-8">
                  {[
                    { depth: 1, label: "ANALYZING_SPECTRUM", content: `<span class="text-yellow-300 font-black block mb-2">💡 Tips Steganografi</span><span class="text-gray-400 block normal-case not-italic">Gunakan panel spektrum di kiri untuk memfilter layer noise digital. Pesan rahasia akan muncul perlahan jika kalibrasi warnanya tepat. Jika angka pada panel tuner berubah warna menjadi <span class='text-yellow-400 font-bold text-glow'>Kuning Menyala</span>, berarti Anda sudah dekat dengan nilai yang tepat.</span>` },
                    { depth: 2, label: "MASTER_OVERRIDE", content: "Intel dikonfirmasi. Berikut adalah nilai kalibrasi yang tepat untuk mengungkap flag: <span class='text-yellow-400 font-mono font-black'>ALPHA: 85</span>, <span class='text-yellow-400 font-mono font-black'>BETA: 42</span>, <span class='text-yellow-400 font-mono font-black'>GAMMA: 175</span>, <span class='text-yellow-400 font-mono font-black'>SIGMA: 99</span>." }
                  ].map((h, i) => (
                    <div key={i} className={`p-4 rounded-2xl border transition-all ${hintStage >= h.depth ? 'bg-yellow-500/10 border-yellow-500/40 text-gray-200' : 'bg-black/40 border-white/5 text-gray-600'}`}>
                       <div className="flex justify-between items-center mb-2">
                          ...
                          {hintStage >= h.depth ? <Check className="w-3 h-3 text-yellow-500" /> : <Lock className="w-3 h-3 text-gray-700" />}
                       </div>
                       {hintStage >= h.depth ? (
                         <p className="text-xs leading-relaxed italic" dangerouslySetInnerHTML={{ __html: h.content }} />
                       ) : (
                         <button onClick={() => unlockHintStage(h.depth)} className="w-full py-2 bg-yellow-600 hover:bg-yellow-500 text-white text-[10px] font-black uppercase tracking-widest rounded-lg transition-all">UNLOCK Intel Layer {h.depth}</button>
                       )}
                    </div>
                  ))}
               </div>
        ...
        {showExitModal && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
             ...
             <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ opacity: 0 }} className="relative bg-gray-900 border border-yellow-500/30 rounded-3xl p-8 max-w-md w-full text-center shadow-2xl">
                ...
                <p className="text-xs text-gray-500 uppercase italic mb-8 leading-relaxed">PROGRESS LOG AKAN TERHAPUS DAN INTERSEPSI AKAN DIPUTUSKAN.</p>
                <div className="flex gap-4">
