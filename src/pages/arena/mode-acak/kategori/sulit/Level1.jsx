...
const SulitLevel1 = () => {
  ...
  const [hintStage, setHintStage] = useState(() => parseInt(localStorage.getItem('ctf_sulit_level1_hint_stage') || '0'));
  ...
  const handleHintClick = () => {
    if (!showHint) {
      ...
      if (!hintStage > 0) {
        ...
        localStorage.setItem('ctf_sulit_level1_hint_used', 'true');
      }
    } else {
      setShowHint(false);
    }
  ...
  const handleSubmit = (e) => {
    ...
    if (inputFlag === realFlag) {
      ...
      const currentBestStars = stats['sulit-1']?.stars || 0;
      const currentBestTimeSec = (() => { 
        const t = stats['sulit-1']?.bestTime; 
        if (!t) return 999999;
      ...
      if (isBetter) {
         stats['sulit-1'] = { stars: stars, bestTime: timeTakenStr, conditions: [true, hintStage < 1, hintStage < 2, hintStage < 3, !isTimeFailed] };
         localStorage.setItem('ctf_mode_acak_stats', JSON.stringify(stats));
      }
      localStorage.removeItem('ctf_sulit_level1_time');
      localStorage.removeItem('ctf_sulit_level1_stars');
      localStorage.removeItem('ctf_sulit_level1_hint_used');
      localStorage.removeItem('ctf_sulit_level1_overtime');
  ...
  const unlockHintStage = (stage) => {
    ...
    setHintStage(stage);
    localStorage.setItem('ctf_sulit_level1_hint_stage', stage.toString());
  };
  ...
  const handleExit = () => {
    ...
    localStorage.removeItem('ctf_sulit_level1_hint_used');
    localStorage.removeItem('ctf_sulit_level1_overtime');
    navigate('/ctf-arena/mode-acak', { state: { returnToLevel: 'sulit-1' } });
  };
      ...
      <div className="min-h-screen bg-gray-950 flex flex-col items-center justify-center p-6 font-mono text-white relative overflow-hidden">
        ...
        <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="z-10 bg-gray-900 border-4 border-red-500 p-12 rounded-3xl text-center max-w-xl shadow-[0_0_50px_rgba(239,68,68,0.3)]">
          <motion.div animate={{ rotate: 360 }} transition={{ duration: 20, repeat: Infinity, ease: "linear" }} className="w-24 h-24 bg-red-500/10 rounded-full flex items-center justify-center mx-auto mb-8 border border-red-500/30">
            <Zap className="w-12 h-12 text-red-500 fill-red-500 drop-shadow-[0_0_10px_#ef4444]" />
          </motion.div>
          <h1 className="text-5xl font-black italic tracking-tighter mb-2 text-white uppercase leading-none">MISSION COMPLETE</h1>
          <p className="text-red-500 font-bold tracking-[0.3em] mb-4 text-[10px]">SECTOR: DATABASE_VAULT // DATA_EXTRACTED</p>
          ...
          <button onClick={() => navigate('/ctf-arena/mode-acak', { state: { returnToLevel: 'sulit-1' } })} className="w-full bg-red-500 text-black font-black py-4 rounded-xl hover:bg-red-500 transition-all text-sm tracking-widest uppercase shadow-[0_10px_30px_rgba(239,68,68,0.2)]">KEMBALI KE MAP TAKTIS</button>
        </motion.div>
      </div>
    );
  }
      ...
      <div className="max-w-7xl mx-auto w-full flex-1 flex flex-col relative z-10">
          ...
          <div className="flex gap-4 items-start z-10">
            ...
            <div>
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-[10px] font-black uppercase tracking-[0.4em] text-red-500/80">TARGET: SECTOR_DATABASE</span>
              </div>
              <h1 className="text-3xl font-black text-white italic tracking-tighter uppercase leading-none shadow-red-500/20">
                LEVEL 8: <span className="text-red-500 drop-shadow-[0_0_10px_#ef4444]">INFILTRASI DATABASE</span>
              </h1>
            </div>
          </div>
        ...
        <div className="flex-1 grid grid-cols-12 gap-6 min-h-0">
          ...
          <div className="col-span-12 lg:col-span-4 flex flex-col gap-6">
            <div className="bg-gray-950/50 border border-red-500/30 rounded-2xl p-6 flex-1 flex flex-col relative overflow-hidden group shadow-[inset_0_0_20px_rgba(239,68,68,0.05)]">
              ...
              <div className="text-xs leading-relaxed text-gray-400 overflow-y-auto pr-2 custom-scrollbar space-y-4 italic">
                ...
                <div className="p-3 bg-red-950/20 border border-red-500/20 rounded-lg mt-4">
            ...
            <AnimatePresence>
              {showHint && (
                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="bg-gray-900/50 border border-white/5 rounded-xl p-4 text-[10px] text-gray-500 italic uppercase leading-tight">
                  <span dangerouslySetInnerHTML={{ __html: `<span className="text-red-300 font-black block mb-2">💡 Apa itu SQL Tautology?</span><span className="text-gray-400 block normal-case not-italic">Serangan SQL Injection yang memasukkan pernyataan logika yang selalu bernilai TRUE. Coba gunakan payload: <span className="text-red-300 font-mono">' OR '1'='1</span>. Karena <span className="text-red-300 font-mono">'1'='1'</span> selalu benar, filter pencarian database dilewati dan seluruh isi tabel akan dikeluarkan (dumped).</span>` }} /></motion.div>
              )}
            </AnimatePresence>
          </div>
          ...
          <div className="col-span-12 lg:col-span-8 flex flex-col gap-6 min-h-0">
             <div className="flex-1 bg-black border border-red-500/20 rounded-2xl flex flex-col overflow-hidden relative group shadow-2xl">
                ...
                <div className="p-6 bg-gray-900/80 border-b border-white/5 backdrop-blur-md">
                   ...
                   <form onSubmit={handleDatabaseSearch} className="flex gap-3">
                      <div className="relative flex-1">
                         <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-red-500/50" />
                         <input
                           type="text"
                           value={queryInput}
                           onChange={(e) => setQueryInput(e.target.value)}
                           placeholder="Search Employee by Name (e.g. Admin)"
                           className="w-full bg-black/60 border border-red-500/30 rounded-xl py-3 pl-12 pr-4 text-sm font-mono text-red-100 placeholder:text-gray-700 focus:outline-none focus:border-red-500 focus:shadow-[0_0_15px_rgba(239,68,68,0.3)] transition-all"
                         />
                      </div>
                      <button 
                        type="submit" 
                        disabled={isSearching}
                        className="bg-red-500 hover:bg-red-500 disabled:bg-gray-800 disabled:text-gray-500 text-black font-black px-8 py-3 rounded-xl uppercase tracking-widest text-xs transition-colors flex items-center justify-center min-w-[120px]"
                      >
                         {isSearching ? <Activity className="w-4 h-4 animate-spin" /> : "QUERY"}
                      </button>
                   </form>
                   {queryError && (
                     <motion.div initial={{ opacity: 0, y: -5 }} animate={{ opacity: 1, y: 0 }} className="mt-3 text-[10px] font-bold text-red-500 italic flex items-center gap-2">
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-0 bg-gray-950">
                   {isDumped ? (
                      <table className="w-full text-left border-collapse font-mono text-[10px] md:text-sm">
                         <thead className="sticky top-0 bg-gray-900 border-b border-red-500/30 text-red-500/60 uppercase tracking-widest shadow-md z-10">
                            <tr>
                         ...
                         <tbody className="divide-y divide-white/5">
                            {dbData.map((row, i) => (
                               <tr key={i} className="hover:bg-red-500/10 hover:text-white transition-colors cursor-default group">
                                  <td className="py-2.5 px-6 font-bold text-red-500/40 group-hover:text-red-500">{row.id}</td>
                ...
                <div className="h-6 bg-gray-900/50 border-t border-white/5 flex items-center px-4 justify-between">
                   ...
                   <div className={`text-[7px] font-black tracking-[0.2em] uppercase ${isDumped ? 'text-red-500/80 animate-pulse' : 'text-red-500/30'}`}>{isDumped ? 'ABNORMAL_QUERY_DETECTED' : 'DB_CONNECTION_SECURE'}</div>
                </div>
             </div>

             {/* SUBMISSION FORM */}
             <div className="bg-gray-900/40 border border-red-500/30 rounded-2xl p-6 relative overflow-hidden">
                <div className="absolute top-0 right-0 p-4 opacity-10"><Terminal className="w-12 h-12 text-red-500" /></div>
                <form onSubmit={handleSubmit} className="relative z-10 flex flex-col md:flex-row gap-4">
                   <div className="flex-1 relative">
                     <input type="text" value={flag} onChange={(e) => setFlag(e.target.value)} placeholder="Masukkan flag di sini (CTF_{...})" className={`w-full bg-black/60 border-2 rounded-xl py-4 px-6 text-sm tracking-widest text-white placeholder:text-gray-700 focus:outline-none transition-all ${status === 'wrong' || status === 'decoy' ? 'border-red-500 shadow-[0_0_15px_#ef4444]' : 'border-red-500/40 focus:border-red-500 focus:shadow-[0_0_20px_rgba(239,68,68,0.2)]'}`} />
                     <AnimatePresence>
                       ...
                       {status === 'decoy' && <motion.div initial={{ opacity: 0, scale: 0.5 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} className="absolute -top-3 -right-3 bg-red-600 text-[8px] font-black italic px-3 py-1 rounded-full text-white shadow-lg uppercase animate-bounce">DECOY DETECTED: Itu file pancingan!</motion.div>}
                     </AnimatePresence>
                   </div>
                   <button type="submit" className="bg-red-500 hover:bg-red-500 text-black font-black uppercase text-xs tracking-[0.2em] px-10 py-4 rounded-xl transition-all active:scale-95 shadow-[0_10px_30px_rgba(239,68,68,0.3)]">[ SUBMIT FLAG ]</button>
                </form>
        ...
        <div className="mt-8 pt-6 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-4 text-gray-700">
           <div className="flex items-center gap-4 text-[9px] font-black tracking-widest">
              <span className="text-red-500/50">{" > "} SYSTEM: OPERATIONAL</span>
           ...
           <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
             ...
             <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="relative bg-gray-900 border border-red-500/30 rounded-3xl p-8 max-w-lg w-full shadow-[0_0_50px_rgba(239,68,68,0.2)] overflow-hidden">
                ...
                <div className="space-y-4 mb-8">
                   {[
                     { depth: 1, label: "CLUE_ANALYSIS", content: "Karena input disisipkan langsung ke query SQL, coba gunakan tautologi: <span className='text-red-400'>' OR '1'='1</span>." },
                     { depth: 2, label: "RECOVERY_DATA", content: "Gunakan payload tersebut pada field 'Employee Name' untuk mengeluarkan seluruh data user." },
                     { depth: 3, label: "EXPLICIT_DATA", content: "Cari baris yang departemennya berisi CTF{...} di hasil pencarian." }
                   ].map((h, i) => (
                     <div key={i} className={`p-4 rounded-2xl border transition-all ${hintStage >= h.depth ? 'bg-red-500/10 border-red-500/40 text-gray-200' : 'bg-black/40 border-white/5 text-gray-600'}`}>
                        ...
                        {hintStage >= h.depth ? (
                          ...
                          <button onClick={() => { setHintStage(h.depth); localStorage.setItem('ctf_sulit_level1_hint_stage', h.depth.toString()); }} className="w-full py-2 bg-red-600 hover:bg-red-500 text-black text-[10px] font-black uppercase tracking-widest rounded-lg transition-all">UNLOCK EXPLOIT DEPTH {h.depth}</button>
                        )}
                     </div>
                   ))}
                </div>
